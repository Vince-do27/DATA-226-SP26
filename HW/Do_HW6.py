...
def extract(city, longitude, latitude):
    ...
    file_path = f"/tmp/{clean_city}_{date_to_fetch}.csv"
...
with DAG(
    ...
    LATITUDE = float(Variable.get("Latitude"))             # TO DO: Use Variable.get to get latitude value from Airflow Variables
    LONGITUDE = float(Variable.get("Longitude"))            # TO DO: Use Variable.get to get longitude value from Airflow Variables
    CITY = "South Lake Tahoe"               # TO DO: Set the city name to your coordinates' city name
    
    database = "USER_DB_BOA"           # TO DO: Set the database name  
    schema = "raw"
    target_table = "weather_ETL_incremental"       # TO DO: Set the target table name
